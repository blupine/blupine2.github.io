...
- 여기서 `authorizationRequestResolver`의 `resolve` 메소드에 bp를 걸고 전체 과정을 살펴보면 하는짓은 다음과 같다.
    ...
    2. registrationId로 ClientRegistrationRepository에서 ClientRegistration을 가져온다
    3. ClientRegistration에 정의되어 있는 KAKAO의 OAuth2 정보를 가져온다(`authorizationUri`, `tokenUri` 등등)
    4. `authorizationRequestUri`를 만들어서 실제 Kakao 서버로 redirect 수행
...
defaultoauth2userservice에서 Map<String, Object> userAttributes
자료구조에다가 response를 getBody() 해서 다 때려박음
근데 이건 OAut2Provider 별로 표준화되지 않은 response이기 때문에 각 Provider별로 다름
...
================================
TODO : 아래 두개 분석해보기, 어떤 역할을 하는건지 -> access token, refresh token을 어떻게 만지작 거리는지, -> 결과적으로 User 도메인을 어떻게 뽑으면 되는지를 생각해야 함
...
스프링 시큐리티의 OAuth2 Client는 OAuth2AuthorizedClientService를 이용해서 필요한 시점에 access token에 접근을 하는 것으로 보임
그리고 refresh token도, 아래 세 개의 메소드를 이용해서 알아서 해주는게 아닐까..?
